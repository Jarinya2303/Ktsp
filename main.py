...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not check_password_hash(user.password, password):
            flash("อีเมลหรือรหัสผ่านไม่ถูกต้อง", 'danger')  # ใช้ flash สำหรับข้อความผิดพลาด
        ...
        # ตรวจสอบประเภทผู้ใช้
        if email == 'charoen@charoen.com':  # แอดมินที่สามารถแก้ไขและลบได้
            return redirect(url_for('admin_dashboard'))
        elif email.endswith('@charoen.com'):  # แอดมินที่สามารถแก้ไขได้แต่ลบไม่ได้
            return redirect(url_for('admin_dashboard'))
        elif email.endswith('@driver.com'):  # คนขับรถ
